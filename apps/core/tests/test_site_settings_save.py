...
class TestSiteSettingsSaveUpdateFields(TestCase):
    def test_update_fields_respected_no_clobber(self):
        ...
        self.assertTrue(reloaded.store_enabled)
        ...
        # que só quer desligar store_enabled
        ...
        stale_b.store_enabled = False
        stale_b.services_enabled = True  # valor stale em memória!
        stale_b.save(update_fields=["store_enabled"])

        # Verifica que services_enabled PERMANECE False (não foi clobberado de volta para True)
        reloaded = SiteSettings.load()
        self.assertFalse(reloaded.store_enabled)
        self.assertFalse(reloaded.services_enabled, "services_enabled foi clobberado de volta para True!")
    ...
    def test_updated_at_updated_on_partial_save(self):
        ...
        obj.store_enabled = False
        obj.save(update_fields=["store_enabled"])
        reloaded = SiteSettings.load()
